...
#  before_action :authenticate, except: :refresh_alert_list
  ...
  def vibration_chart
    @result = WindTurbineVibrationMeasurement.where('created_at >= ?', 1.day.ago.change(hour: 0, min: 0, sec: 0)).order(:created_at).select(:max_ejex, :m_ejex, :min_ejex, :max_ejey, :m_ejey, :min_ejey, :max_ejez, :m_ejez, :min_ejez, :created_at)
    timestamp =  @result.pluck(:created_at)
    timestamp.collect! { |element| element.strftime("%F %T") }
